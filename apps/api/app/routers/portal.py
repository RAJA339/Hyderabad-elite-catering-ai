...
async def _quote_bundle(quote: dict) -> dict:
    ...
    payments = await db.fetch("SELECT kind, amount, status::text AS status, payment_link, paid_at FROM payments WHERE quote_id IN (SELECT id FROM quotes WHERE quote_number=$1 AND tenant_id=$2) ORDER BY created_at", quote["quote_number"], quote["tenant_id"])
    ...
    return {
        ...
        "lock": dict(lock) if lock else None, "chat": [dict(m) for m in reversed(msgs)],
    }

...
2. Set the root directory to `apps/api`. Railway detects the Dockerfile.
3. Add these variables:
   ...
   | `APP_ENV` | `prod` |
   | `CORS_ORIGINS` | your Vercel URL, filled in after step 5 |
   | `PUBLIC_WEB_URL` | the same Vercel URL |
   | `RUN_SCHEDULER` | `1` on exactly one instance, `0` on any others |

4. Deploy, then open `https://<your-api>.up.railway.app/health`. It should report `db: true`.
